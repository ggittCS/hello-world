...
def maximize_probability_of_favor(G, v1, v2):
    ...
    for node in G:
        ...
        for link in G[node]:
            P = G[node][link]
            if P < 1e-323:   # make sure log(P) doesn't crash
                newG[node][link] = float('inf')
            else:
                newG[node][link] = -math.log(P)
    if (edges * math.log(nodes, 2)) < (nodes**2):
        R = dijkstra_heap(newG, v1)   # m * log(n) is smallest
    else:
        R = dijkstra_list(newG,v1)    # n**2 is smallest
    if v2 not in R:
        return None, 0.0
    next = v2
    path = [v2]
