...
tsv = csv.reader(open("Marvel_characters.txt"), delimiter = '\t')
	...
	while len(dist_so_far) > 0:
		...
		for x in graph[node]:
			...
			if x not in final_dist:
				...
				# this comparison uses the fact that
				# tuples are comparable
				elif new_entry < dist_so_far[x]:
					# the new distance is less than the
					# best known
...
def find_path(dist, target):
	...
	while True:
		...
		if prev is None:
			# we've reached our target, so return
			# the path
			return path
		path.append(prev)
		node = prev
...
for char1 in chars:
	...
	for char2 in char_dist:
		...
		# if the weighted path is longer than the hop path, we need
		# to save it
