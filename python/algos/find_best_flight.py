...
def make_link(G, num,city1, city2, flight_duration, departure, arrival, cost):
    if city1 not in G:
        G[city1] = {}
...
def find_best_flights(flights, origin, destination):
    G = create_connection_graph(all_flights)
    heap = []
        ...
        for x in G[w]:
            for flight in G[w][x]:
                ...
                if dept > farrv:
                    ...
                    heapq.heappush(heap, (new_cost, new_durn, arrv, new_flights, new_path))
    return None
