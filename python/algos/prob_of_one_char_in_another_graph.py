...
def create_weighted_graph(bipartiteG, characters):
    G = {}
    # your code here
    keys = set(bipartiteG.keys())
    comics = list(set(keys) - set(characters))
    for char1 in characters:
        G[char1] = {}
        comics1 = set(bipartiteG[char1].keys())
        for char2 in characters:
            if char1 is char2:
                continue
            comics2 = set(bipartiteG[char2].keys())
            G[char1][char2] = 1.0*len(set(comics1 & comics2))/len(comics1 | comics2)
            if G[char1][char2] == 0.0:
                G[char1][char2] = None
    return G
